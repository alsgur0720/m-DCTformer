...
class Swin2SR(nn.Module):
    ...
    def __init__(self, opt, img_size=64, patch_size=1, in_chans=3,
    ...
    def forward(self, x, outH, outW):
        ...
        if self.upsampler == "pixelshuffle_aux":
            ...
            # return self.h2a2sr(x[:, :, :H*self.upscale, :W*self.upscale],outH,outW)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.weight_x_k = nn.Conv2d(dim, dim, kernel_size=(3,1), stride=1, padding=(1,0), groups=dim, bias=bias)
        self.weight_y_k = nn.Conv2d(dim, dim, kernel_size=(1,3), stride=1, padding=(0,1), groups=dim, bias=bias)
        self.weight_x_v = nn.Conv2d(dim, dim, kernel_size=(3,1), stride=1, padding=(1,0), groups=dim, bias=bias)
        self.weight_y_v = nn.Conv2d(dim, dim, kernel_size=(1,3), stride=1, padding=(0,1), groups=dim, bias=bias)
    ...
    def forward(self, x):
        ...
        coef = 1.0
        w_x_k = self.weight_x_k(k)*coef
        w_y_k = self.weight_y_k(k)*coef
        w_x_v = self.weight_x_v(v)*coef
        w_y_v = self.weight_y_v(v)*coef
